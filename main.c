 ...
 * boundaries.
 *
 * Copy single word into *word (Maximum length of each word is MAX_STRING=100)
 *
 * Parameters:
...
void ReadWord(char *word, FILE *fin) {
    ...
    while (!feof(fin)) {
        ...
        if ((ch == ' ') || (ch == '\t') || (ch == '\n')) {
            ...
            // Put the newline(\n) back before returning... (because we want to detect end of sentence we put it back)
            if (a > 0) {
                if (ch == '\n')
                    ungetc(ch, fin);//Push a character back onto an input stream
                break;
            }
...
void SortVocab() {
    ...
    for (a = 0; a < size; a++) {
        ...
        if ((vocab[a].cn < min_count) && (a != 0)) {
            ...
            vocab_hash[hash] = a; // vocab_hash is actually a look-up table that returns index of that word
...
void InitNet() {
    ...
    // posix_memalign(&p, 32, 128) will be a 128-byte chunk of memory
    // whose start address is guaranteed to be a multiple of 32.
    ...
    // next_random is assigned a value by a function which looks like a Linear Congruential Generator.
    // X_{n+1}= (aX_{n} + c) mod m
    // c = 11, a = 25214903917, m = 2^48, output bit: 0xFFFF
    // ref[1]: https://en.wikipedia.org/wiki/Linear_congruential_generator
    // ref[2]: http://www.public.iastate.edu/~mervyn/stat580/Notes/s09rn.pdf
    for (a = 0; a < vocab_size; a++)
        for (b = 0; b < layer1_size; b++) {
 ...
 * default number of threads equals to 12 (num_threads=12)
 */
void TrainModel() {
    long a, b, c, d;
    FILE *fo;
...
/**
 * find correct index of *str (-size, -train,...)
 * @param str args string
 * @param argc length of args
 * @param argv 2-D contains all args
...
int main(int argc, char **argv) {
    ...
    // we should set some parameter for training our vectors
    // below is list of all of them
    // argc == 1 means that no args passed to main function
    if (argc == 1) {
    ...
    // if already something exist, make it clean
    output_file[0] = 0;
    save_vocab_file[0] = 0;
    read_vocab_file[0] = 0;

    //size of word vectors (hidden layer); default is 100
    if ((i = ArgPos((char *) "-size", argc, argv)) > 0) // find index of -size in list of argv
        layer1_size = atoi(argv[i + 1]); //next index of i to fetch size value // atoi : convert String to Integer

    //text data from <file> to train the model
    ...
    // learning rate for cbow is 0.05, skip-gram is 0.025
    if (cbow) alpha = 0.05;
    ...
    //sliding window from target word, default is 5
    if ((i = ArgPos((char *) "-window", argc, argv)) > 0)
        window = atoi(argv[i + 1]);

    //Set threshold for occurrence of words. Those that appear with higher frequency in the training data
    //will be randomly down-sampled; default is 1e-3, useful range is (0, 1e-5)
    //there are many stop words or frequent word that not useful for use
    ...
    //The word2vec C code implements an equation for calculating a
    // probability with which to keep a given word in the vocabulary
    // (sqrt (x/0.001) + 1)*0.001/x
    // default value of x = 0.001
    ...
    //Number of threads, default is 12
